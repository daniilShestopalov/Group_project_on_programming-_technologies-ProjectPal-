...
public interface ProjectRepository extends JpaRepository<Project, Integer> {
    ...
    @Query("SELECT p FROM Project p JOIN StudentProject sp ON p.id = sp.project.id WHERE sp.student.id = :studentId AND p.startDate >= :startDate AND p.endDate <= :endDate")
